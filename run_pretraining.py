...
def get_masked_lm_output(bert_config, input_tensor, output_weights,project_weights, positions,
  ...
  input_tensor = gather_indexes(input_tensor, positions)    # (batch_size * masked_number, width)
  ...
  with tf.variable_scope("cls/predictions"):
    ...
    log_probs = tf.nn.log_softmax(logits, axis=-1)    # (b * m, v)

    label_ids = tf.reshape(label_ids, [-1])           # (1, b * m)
    label_weights = tf.reshape(label_weights, [-1])   # (1, b * m)  1 -> masked, 0 -> padding

    one_hot_labels = tf.one_hot(label_ids, depth=bert_config.vocab_size, dtype=tf.float32)  # (b * m, v)

    # The `positions` tensor might be zero-padded (if the sequence is too
    # short to have the maximum number of predictions). The `label_weights`
    # tensor has a value of 1.0 for every real prediction and 0.0 for the
    # padding predictions.
    per_example_loss = -tf.reduce_sum(log_probs * one_hot_labels, axis=[-1])  # (b * m, 1)
...
def gather_indexes(sequence_tensor, positions):
  ...
  Args:
    ...
    positions: (batch_size, mask_number).
  """
  ...
  flat_offsets = tf.reshape(
      tf.range(0, batch_size, dtype=tf.int32) * seq_length, [-1, 1])    # [[0]*sl, [1]*sl, ..., [batch_size]*sl] -> (batch_size, 1)
  flat_positions = tf.reshape(positions + flat_offsets, [-1])           # (1, batch_size * mask_number) with offset
  flat_sequence_tensor = tf.reshape(sequence_tensor,              
                                    [batch_size * seq_length, width])   # (batch_size * seq_length, width)
  output_tensor = tf.gather(flat_sequence_tensor, flat_positions)       # at axis 0, select offset positions, i.e. masked words.
